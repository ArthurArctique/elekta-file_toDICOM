...
### Alors pourquoi avoir décodé le format nous-mêmes ?

Le noyau s'appuie sur pymedphys, mais deux outils d'`exploration/` décodent le
TRF octet par octet : `organiser_trf.py`, qui ne veut que quatre colonnes sur
350 et travaille directement sur les octets d'un zip, et `lecteur_trf.html`,
qui décode dans le navigateur pour qu'un fichier réel puisse être ouvert sans
rien transmettre.

C'est **leur** confrontation à pymedphys qui a donné **350/350** et **354/354**
colonnes identiques au bit près. Elle ne valide pas le noyau — il utilise
pymedphys — mais elle a établi la compréhension du format sur laquelle repose
[LA_CHAINE.md](LA_CHAINE.md), et elle a débusqué au passage des conventions qui
ne se devinaient pas : le banc Y2 renvoyé, ses erreurs de position qui ne le
sont pas, les 12 colonnes sans mise à l'échelle.
